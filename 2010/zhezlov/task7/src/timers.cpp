...
Timers::Timers(QWidget *parent)
	...
	l->addWidget(cb);
        cb->setObjectName("TimerSwitch");
	cb->addItem("Timer 1");
	cb->addItem("Timer 2");

	l1 = new QHBoxLayout();
	pb = new QPushButton("Set", this);
        pb->setObjectName("setButton");
...
void Timers::setBtnClk(){
	if ( timerIds[curTimerIndex] ){
        ...
        periods[curTimerIndex] = lEd->text().toInt();
        timerIds[curTimerIndex] = startTimer( periods[curTimerIndex] );
}
